...
registerBlockType( 'kenzap/cta-42', {
	...
	edit: ( props ) => {
        ...
        if ( props.attributes.preview ) return ( <img src={ `${ window.kenzap_cta_path + 'cta-42/preview.jpeg' }` } /> );

        if(!isScriptLoad){

            jQuery.getScript( window.kenzap_cta_path + 'cta-42/script.js', function( data, textStatus, jqxhr ) { ( function( $ ) { launchBars($); }); }( jQuery ) );
